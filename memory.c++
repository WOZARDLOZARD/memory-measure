...
std::string format(double f)
{
    if (f == 0) {
        return "0";
    }
    int d = (int)::ceil(::log10(f < 0 ? -f : f));
    double order = ::pow(10., 3 - d);
    std::stringstream ss;
    ss << std::fixed << std::setprecision(std::max(3 - d, 0)) << custom_round(f * order) / order;
    return ss.str();
}
...
int main() {
    ...
    for (i = 0; i < 20; i++) {
        ...
        used += heap;
        total += used;
        cout << "Iteration " + std::to_string(i + 1) + ": " + format(used / 1e+6) + " MB\n";
    }
    ...
    cout << "\n\nTotal usage: " + format(total / 1e+6) + " MB";
